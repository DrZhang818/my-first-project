...
Info operator + (const Info &l, const Info &r) {
    if(l.sum == 0) return r;
    if(r.sum == 0) return l;
    assert(l.rcolor && l.lcolor && r.lcolor && r.rcolor);
    return {l.sum + r.sum - (l.rcolor == r.lcolor), l.lcolor, r.rcolor};
}
    ...
    while(m--) {
        ...
        if(c == 'C') {
            ...
            while(top[u] != top[v]) {
                ...
                seg.rangeApply(in[top[u]], in[u], {x});
                u = parent[top[u]];
            }
            if(dep[u] > dep[v]) {
                swap(u, v);
            }
            seg.rangeApply(in[u], in[v], {x});
        } else {
            int u, v;
            cin >> u >> v;
            int ans = 0;
            while(top[u] != top[v]) {
                ...
                ans += seg.rangeQuery(in[top[u]], in[u]).sum - (seg.rangeQuery(in[top[u]], in[top[u]]).lcolor == seg.rangeQuery(in[parent[top[u]]], in[parent[top[u]]]).lcolor);
                u = parent[top[u]];
            }
            if(dep[u] > dep[v]) {
                swap(u, v);
            }
            ans += seg.rangeQuery(in[u], in[v]).sum;
            cout << ans << "\n";
        }
    }    
}
