...
Info operator + (const Info &l, const Info &r) {
    ...
    return {max({l.up, r.up, l.mx - r.mn}), max({l.down, r.down, r.mx - l.mn}), min(l.mn, r.mn), max(l.mx, r.mx)};
}
...
void solve() {
    ...
    for(int i = 1; i <= n; i++) {
        seg.modify(in[i], {0, 0, a[i], a[i]});
    }

    auto add = [&](int u, int v, int d) -> void {
        while(top[u] != top[v]) {
            if(dep[top[u]] < dep[top[v]]) {
                swap(u, v);
            }
            seg.rangeApply(in[top[u]], in[u], {d});
            u = parent[top[u]];
        }
        if(dep[u] > dep[v]) {
            swap(u, v);
        }
        seg.rangeApply(in[u], in[v], {d});
    };
    ...
    while(q--) {
        ...
        while(top[l] != top[r]) {
            if(dep[top[l]] > dep[top[r]]) {
                L = seg.rangeQuery(in[top[l]], in[l]) + L;
                l = parent[top[l]];
            } else {
                R = seg.rangeQuery(in[top[r]], in[r]) + R;
                r = parent[top[r]];
            }
        }
        if(dep[l] > dep[r]) {
            L = seg.rangeQuery(in[r], in[l]) + L;
        } else {
            R = seg.rangeQuery(in[l], in[r]) + R;
        }
        int ans = max({L.up, R.down, R.mx - L.mn});
        cout << ans << "\n";
    }
}
