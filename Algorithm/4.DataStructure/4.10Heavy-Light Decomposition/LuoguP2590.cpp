...
Info operator + (const Info &l, const Info &r) {
    return {l.sum + r.sum, l.len + r.len, max(l.mx, r.mx)};
}
    ...
    while(q--) {
        ...
        if(s[0] == 'C') {
            ...
            while(top[u] != top[v]) {
                ...
                mx = max(mx, seg.rangeQuery(in[top[u]], in[u]).mx);
                u = parent[top[u]];
            }
            if(dep[u] > dep[v]) {
                swap(u, v);
            }
            mx = max(mx, seg.rangeQuery(in[u], in[v]).mx);
            cout << mx << "\n";
        } else {
            int u, v;
            cin >> u >> v;
