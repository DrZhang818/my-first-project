...
//+-----------------------------------------------------------------------+
//| Function to retrieve and process indicator data for new Daily candle  |
//+-----------------------------------------------------------------------+
 ...
 if(take_profit == 0 || stop_loss == 0)
 {
      ...
      if(take_profit == 0){
         ...
         if(debugging) Print("Take Profit set to: ", take_profit, " points");
      }
      if(stop_loss == 0){
         stop_loss = dynamic_sl_pips;
         if(debugging) Print("Stop Loss set to: ", stop_loss, " points");
      }
 }

if(HasPendingStopOrder(Symbol())){
 if(HasPendingStopOrderInCurrentCandle(Symbol(), inputTimeframe))
      if(debugging) Print("Pending stop order was placed in the current candle.");
 else
   {
      CancelPendingOrders(trade, Symbol(), debugging);
   }
}
