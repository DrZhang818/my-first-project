...
void solve() {  
    ...
    for(int i = 1; i <= n; i++) {
        int y = -1;
        while(!stk.empty() && (a[i] > a[stk.top()] || a[i] == a[stk.top()] && c[i] < c[stk.top()])) {
            y = stk.top();
            stk.pop();
        }
        if(y != -1) {
    ...
    [&](this auto &&self, int u, int fa) -> void {
        ...
        in[u] = ++timer;
        if(tr[u].l) {
            c[tr[u].l] = min(c[tr[u].l], c[u]);
            self(tr[u].l, u);
        }
        if(tr[u].r) {
            c[tr[u].r] = min(c[tr[u].r], c[u]);
            self(tr[u].r, u);
        }
        out[u] = timer;
    } (rt, 0);
    ...
    ll ans = seg.rangeQuery(1, n).sum - c[rt];
    cout << ans << " ";
    for(int i = 1; i <= n; i++) {
        int x = bel[p[i]];
        c[x] = 0;
        seg.rangeApply(in[x], out[x], {1});
        ans = seg.rangeQuery(1, n).sum - c[rt];
        cout << ans << " \n"[i == n];
    }
}
