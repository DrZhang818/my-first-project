...
void solve() {
    ...
    for(int i = n, j = n; i >= 1; i--) {
        tot += seg.rangeQuery(1, a[i] - 1).sum;
        seg.rangeApply(a[i], n, {1});
    }

    LazySegmentTree<Info, Tag> seg2(n + 1);
    i64 ans = tot;
    for(int i = 1; i <= n; i++) {
        seg2.rangeApply(a[i], a[i], {1});
    }
    for(int i = n; i >= 1; i--) {
        seg2.rangeApply(a[i], a[i], {-1});
        ans = max(ans, tot + seg2.rangeQuery(a[i], n).sum);
    }
