...
template<class T>
void polarSort(std::vector<Point<T>>& p, const Point<T>& o = Point<T>()) {
...
template<class T>
std::vector<int> getPolarOrder(const std::vector<Point<T>>& pts, const Point<T>& o = Point<T>()) {
    int n = pts.size();
    std::vector<int> p(n);
...
template<class T>
Point<T> FootPoint(Point<T> p, Line<T> l) {
    Point<T> x = p - l.a, y = p - l.b, z = l.b - l.a;
    db len1 = dot(x, z) / length(z);
    return l.a + z * (len1 / length(z));
}
int f[5000],m,n;
double a,r,res[5000];
Point<double>p[5000],q[5000];
double coord(Point<double>a,Point<double>b,Point<double>c){
    return dot(b-a,c-a)>0.0?length(c-a):-length(c-a);
}
int main(){
    ...
    for(int i=0;i<n;i++){
        Line<double>l=Line(p[i],p[(i+1)%n]);
        vector<pair<double,double>>s;
        for(int j=0;j<m;j++){
            if(distancePL(q[j],l)<r){
                Point<double>qq=FootPoint(q[j],l);
                double len=sqrt(r*r-distancePL(q[j],l)*distancePL(q[j],l));
                Point<double>d=normalize(p[(i+1)%n]-p[i]);
                double u=coord(p[i],p[(i+1)%n],qq-len*d);
                len=distancePL(q[j],l)/tan(a);
                double v=coord(p[i],p[(i+1)%n],qq-len*d);
                u=fmax(coord(p[i],p[(i+1)%n],p[i]),u);
                v=fmin(coord(p[i],p[(i+1)%n],p[(i+1)%n]),v);
                if(u<v){
        ...
        vector<double>b;
        b.push_back(coord(p[i],p[(i+1)%n],p[i]));
        b.push_back(coord(p[i],p[(i+1)%n],p[(i+1)%n]));
        ...
        for(int i=0;i<s.size();i++){
            int lf=lower_bound(b.begin(),b.begin()+sz,s[i].first-1e-9)-b.begin(),rt=lower_bound(b.begin(),b.begin()+sz,s[i].second-1e-9)-b.begin();
            f[lf+1]++;
            f[rt+1]--;
        }
        for(int i=1;i<=sz;i++){
